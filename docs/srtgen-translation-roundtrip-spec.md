...
- 「照合」ボタン → パース・検証を実行し、結果を表示（あくまで任意のプレビュー機能。適用の前提条件ではない）
- 「適用」ボタン → 押下時点の textarea の内容を**そのつど再パース**して適用する（「照合」時のプレビュー結果をキャッシュとして使い回さない）。貼り付けた内容を常に正とし、貼り付けに含まれる番号のクリップは無条件に上書きする。貼り付けに含まれない番号のクリップは現状維持（消さない）。「空欄のみ埋める/すべて上書き」の2モードは廃止した（2回目以降の貼り付けで既存訳文が残ってしまう不具合の原因だったため）。正当な番号(1..N)のエントリが1件も取れない場合のみ適用を拒否する（5節参照）。
